...
# This creates fewer branches that group related features together
...
# 1. Infrastructure & PWA Setup
echo "📦 Creating feature/infrastructure-setup..."
git checkout -b feature/infrastructure-setup 2>/dev/null || git checkout feature/infrastructure-setup
git checkout "$BASE_BRANCH" -- package.json vite.config.js tailwind.config.js postcss.config.js .gitignore src/index.css index.html public/manifest.json public/icons/ src/main.jsx
git add .
git commit -m "feat: infrastructure setup - Tailwind, Vite, PWA configuration" || echo "Branch already has changes"
...
echo ""
echo "✅ Created 7 logical feature branches:"
echo "   1. feature/infrastructure-setup"
